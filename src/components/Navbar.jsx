...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'py-3 sm:py-4 glass shadow-lg shadow-black/20' : 'py-4 sm:py-6 bg-transparent'
    }`}>
      <div className="max-w-7xl mx-auto px-4 xs:px-6 sm:px-8 md:px-12 flex justify-between items-center">
        {/* Logo */}
        <a href="#" className="text-lg xs:text-xl font-bold tracking-tight text-gradient">
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden absolute top-full left-0 right-0 glass border-t border-white/10 transition-all duration-300 ease-in-out overflow-hidden ${
        isOpen 
          ? 'max-h-[400px] opacity-100 py-4 sm:py-6' 
